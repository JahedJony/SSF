	...
	*/
	function getUserGroupPermission($action_id){	
		...
		foreach($logged_user_groups as $row){
			$status_return = $this->getSingleRow("select status from user_group_permission where group_id = '$row[group_id]' and action_id = $action_id");
			return 	$status_return['status'];
		}		
	}
	...
	function getPatientDetails($health_card_no){
		...
		$health_card_details = $this->getSingleRow("select * from helth_card where card_no= '$health_card_no'");
