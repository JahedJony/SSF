...
<div class="x_content">
	<div class="x_panel employee_profile">
		...
		<div class="x_content" id="iniial_collapse">
			<br />     
			<form method="emp"  id="dc_update_form" name="dc_info_form" enctype="multipart/form-data" class="form-horizontal form-label-left">   
				<div class="row">
					<div class="col-md-9">	
						...
						<div class="form-group">
							...
							<div class="col-md-4 col-sm-4 col-xs-6">
								<input type="text" id="dc_name" name="dc_name" required class="form-control col-lg-12" readonly="readonly"/>
							</div>	
						...
						<div class="form-group">
							<label class="control-label col-md-2 col-sm-2 col-xs-6">Contact Person</label>
							<div class="col-md-4 col-sm-4 col-xs-6">
								<input type="text" id="contact_person_name" name="contact_person_name" required class="form-control col-lg-12" readonly="readonly"/>
							</div>	
						...
						<div class="form-group">
							...
							<div class="col-md-2 col-sm-2 col-xs-12">
								<button  type="submit" id="save_dc_info" class="btn btn-success">Update</button>                                        
							</div>
					...
					<div class="col-md-3">
						...
						<img src="" class="img-thumbnail" id="dc_img">
					</div>
				</div>
			</form> 
		</div>
...
$(document).ready(function () {	
	...
	load_dc_profile = function load_dc_profile(){
		...
		$.ajax({
			...
			async:false,
			data: {
				q: "get_diagnostic_info",
				dc_id: "<?php echo $_SESSION['user_id']; ?>"
			},
			success: function(data){
				$('#dc_id').val(data.records.id);
				$('#dc_name').val(data.records.name);
				$('#user_name').val(data.records.username);
				$('#contact_person_name').val(data.records.contact_person_name);
				$('#contact_no').val(data.records.mobile_no);	
				$('#email').val(data.records.email);
				$('#dc_img').attr("src",project_url+data.records.image);
				$('#dc_img').attr("width", "70%","height","70%");
	...
	$('#save_dc_info').click(function(event){		
		event.preventDefault();
		var formData = new FormData($('#dc_update_form')[0]);
		...
		if($.trim($('#old_password').val()) == ""){
			...
			$.ajax({
				...
				success: function(data){
					$('#save_dc_info').removeAttr('disabled','disabled');
					if(data>0){
