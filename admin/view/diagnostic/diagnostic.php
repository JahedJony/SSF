...
<div class="x_panel">
    ...
    <div class="x_content">
				...
				<div class="row advance_search_div alert alert-warning">
					<div class="row">
						...
						<div class="col-md-2 col-sm-2 col-xs-6">
							<input class="form-control input-sm" type="text" name="diag_speciality" id="diag_speciality"/> 
						</div>
						<label class="control-label col-md-1 col-sm-1 col-xs-6" style="text-align:right">Address</label>
						<div class="col-md-2 col-sm-2 col-xs-6">
							<input class="form-control input-sm" type="text" name="diag_address" id="diag_address"/> 
						</div>
    	...
    	<div class="dataTables_length">
        	<label>Show 
                <select size="1" style="width: 56px;padding: 6px;" id="diagnostic_Table_length" name="diagnostic_Table_length" aria-controls="diagnostic_Table">
         ...
         <div class="dataTables_filter" id="diagnostic_Table_filter">         
			<div class="input-group">
                <input class="form-control" id="search_diagnostic_field" style="" type="text">
                <span class="input-group-btn">
                    <button type="button" class="btn btn-primary has-spinner" id="search_diagnostic_button">
       ...
       <div style="height:250px; width:100%; overflow-y:scroll">
        <table id="diagnostic_Table" name="table_records" class="table table-bordered  responsive-utilities jambo_table table-striped  table-scroll ">
            <thead>
                <tr class="headings">
            ...
            <tbody id="diagnostic_table_body" class="scrollable">              
                
            </tbody>
        </table>
        </div>
        <div id="diagnostic_Table_div">
            <div class="dataTables_info" id="diagnostic_Table_info">Showing <span id="from_to_limit"></span> of  <span id="total_record"></span> entries</div>
            <div class="dataTables_paginate paging_full_numbers" id="diagnostic_Table_paginate">
            </div> 
        </div>  
    </div>
</div>
...
<div class="x_panel employee_entry_cl">
    ...
    <div class="x_content" id="iniial_collapse">
		...
		<form method="post"  id="diagnostic_form" name="diagnostic_form" enctype="multipart/form-data" class="form-horizontal form-label-left">   
			<div class="row">
				<div class="col-md-9">
					<div class="form-group">
						<label class="control-label col-md-2 col-sm-2 col-xs-6">Full Name<span class="required">*</span></label>
						<div class="col-md-4 col-sm-4 col-xs-12">							
							<input type="text" id="diagnostic_name" name="diagnostic_name" required class="form-control col-lg-12"/>
						</div>
						<label class="control-label col-md-2 col-sm-2 col-xs-6">Contact Person<span class="required">*</span></label>
						<div class="col-md-4 col-sm-4 col-xs-12">							
							<input type="text" id="contact_person_name" name="contact_person_name" required class="form-control col-lg-12"/>
						</div>
					</div>
					<div class="form-group">
				...
				<div class="col-md-3">
					<img src="<?php echo $activity_url ?>images/no_image.png" width="70%" height="70%" class="img-thumbnail" id="diagnostic_img">
					<input type="file" name="diagnostic_image_upload" id="diagnostic_image_upload">
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-md-3 col-sm-3 col-xs-12">
					...
					<button type="submit" id="save_diagnostic_info" class="btn btn-success">Save</button>                    
...
$(document).ready(function () {	
	...
	load_diagnostic_grid = function load_diagnostic_grid(search_txt){
		$("#search_diagnostic_button").toggleClass('active');		 
		var diagnostic_Table_length =parseInt($('#diagnostic_Table_length').val());
		 var diag_speciality = $("#diag_speciality").val();	
		    var diag_address = $("#diag_address").val();	
		var active_status = $("input[name=is_active_status]:checked").val();
		
		$.ajax({
			url: project_url+"controller/diagnosticController.php",
			dataType: "json",
			type: "post",
			async:false,
			data: {
				q: "grid_data",
				diag_speciality:diag_speciality,
				diag_address:diag_address,
				active_status:active_status,
				search_txt: search_txt,
				limit:diagnostic_Table_length,
			...
			success: function(data) {		
				...
				if($.trim(search_txt) == "Print"){
					...
					if(diag_speciality != "")   serach_areas += "Specality: "+diag_speciality+"  <br>";									
					if(diag_address != "")   serach_areas += "Address: "+diag_address;
					/*<input name="print" type="button" value="Print" id="printBTN" onClick="printpage();" />*/
					
					html +='<div width="100%"  style="text-align:center"><img src="'+employee_import_url+'/images/logo.png" width="80"/></div><h2 style="text-align:center">Shastho Shikkha Foundation</h2><h4 style="text-align:center">Diagnostic Information Report</h4><table width="100%"><tr><th width="60%" style="text-align:left"><small>'+serach_areas+'</small></th><th width="40%"  style="text-align:right"><small>Printed By: '+user_name+', Date:'+todate+'</small></th></tr></table>';
					if(!jQuery.isEmptyObject(data.records)){
						html +='<table width="100%" border="1px" style="margin-top:10px;border-collapse:collapse"><thead><tr><th style="text-align:left">Name</th><th style="text-align:left">Speciality</th><th style="text-align:left">Contact Person</th><th style="text-align:center">Mobile No</th><th style="text-align:left">Address</th><th style="text-align:left">Email</th></tr></thead><tbody>';
						$.each(data.records, function(i,data){  
							html += "<tr>";				
							html +="<td style='text-align:left'>"+data.name+"</td>";
							html +="<td style='text-align:left'>"+data.speciality+"</td>";
							html +="<td style='text-align:left'>"+data.contact_person_name+"</td>";
					...
					WinId = window.open("", "Diagnostic Report","width=950,height=800,left=50,toolbar=no,menubar=YES,status=YES,resizable=YES,location=no,directories=no, scrollbars=YES"); 
					WinId.document.open();
					...
					show_record_no(current_page_no, diagnostic_Table_length, data.total_records )
					
					var total_pages = data.total_pages;	
					var records_array = data.records;
					$('#diagnostic_Table tbody tr').remove();
					//$("#search_diagnostic_button").toggleClass('active');
					if(!jQuery.isEmptyObject(records_array)){
						...
						create_set_grid_table_row(records_array,colums_array,condition_array,"diagnostic","diagnostic_Table", 0);
						// show the showing no of records and paging for records 
						$('#diagnostic_Table_div').show();					
						// code for dynamic pagination 				
						paging(total_pages, current_page_no, "diagnostic_Table" );					
					}
					// if the table has no records / no matching records 
					else{
						grid_has_no_result("diagnostic_Table",8);
					}
				}
				$("#search_diagnostic_button").toggleClass('active');				
			}
	...
	load_page = function load_page(page_no){
		if(page_no != 0){
			...
			var search_txt = $("#search_diagnostic_field").val();
			load_diagnostic_grid(search_txt)
		}
	}	
	// function after click search button 
	$('#search_diagnostic_button').click(function(){
		var search_txt = $("#search_diagnostic_field").val();
	...
	$('#search_diagnostic_field').keypress(function(event){
		var search_txt = $("#search_diagnostic_field").val();	
		if(event.keyCode == 13){
			// every time current_page_no need to set to "1" if the user search from search bar
			current_page_no=1;
...
*/
$(document).ready(function () {		
	var url = project_url+"controller/diagnosticController.php";
	
	
	//advance search
	$('#adv_search_button').click(function(){
		load_diagnostic_grid("Advance_search");
	});
	
	//print advance search data
	$('#adv_search_print').click(function(){
		load_diagnostic_grid("Print");
	});
	
	
	// save and update for public post/notice
	$('#save_diagnostic_info').click(function(event){		
		event.preventDefault();
		var formData = new FormData($('#diagnostic_form')[0]);
		formData.append("q","insert_or_update");
		if($.trim($('#diagnostic_name').val()) == ""){
			success_or_error_msg('#form_submit_error','danger',"Please Insert Name","#diagnostic_name");			
		}
		else if($.trim($('#user_name').val()) == ""){
			success_or_error_msg('#form_submit_error','danger',"Please Insert User Name","#diagnostic_name");
		}
		else{
		//	$('#save_diagnostic_info').attr('disabled','disabled');
			$.ajax({
				url: url,
				type:'POST',
				data:formData,
				...
				success: function(data){
					$('#save_diagnostic_info').removeAttr('disabled','disabled');
	...
	clear_form = function clear_form(){			 
		$('#master_id').val('');
		$("#diagnostic_form").trigger('reset');		
		$('#diagnostic_img').attr("width", "0%");
		$('#diagnostic_img').attr("src",project_url+"images/no_image.png");
		$('#diagnostic_img').attr("width", "70%","height","70%");
		$('#img_url_to_copy').val("");
		$('#diagnostic_form').iCheck({
			checkboxClass: 'icheckbox_flat-green',
			radioClass: 'iradio_flat-green'
		});	
		$("#diagnostic_form .tableflat").iCheck('uncheck');
		$('#save_diagnostic_info').html('Save');
	}
	...
	delete_diagnostic = function delete_diagnostic(diagnostic_id){
		if (confirm("Do you want to delete the record? ") == true) {
			$.ajax({
				url: url,
				type:'POST',
				async:false,
				data: "q=delete_diagnostic&diagnostic_id="+diagnostic_id,
				success: function(data){
					if($.trim(data) == 1){
	...
	edit_diagnostic = function edit_diagnostic(diagnostic_id){
		$.ajax({
			url: project_url+"controller/diagnosticController.php",
			dataType: "json",
			type: "post",
			...
			success: function(data){
				if(!jQuery.isEmptyObject(data.records)){
					$.each(data.records, function(i,data){ 					
						...
						$('#diagnostic_img').attr("src",project_url+data.image);
						$('#diagnostic_img').attr("width", "70%","height","70%");
						...
						//change button value 
						$('#save_diagnostic_info').html('Update');
