	...
	load_bill_grid = function load_bill_grid(){		
		...
		$.ajax({
			...
			success: function(data) {
				...
				html +='<div width="100%"  style="text-align:center"><img src="'+employee_import_url+'/images/logo.png" width="80"/></div><h2 style="text-align:center">Shastho Shikkha Foundation</h2><h4 style="text-align:center">Bill Vs Payment Report</h4><table width="100%"><tr><th width="60%" style="text-align:left"><small>'+serach_areas+'</small></th><th width="40%"  style="text-align:right"><small>Printed By: '+user_name+', Date:'+todate+'</small></th></tr></table>';
				
				if(!jQuery.isEmptyObject(data.records)){
					html +='<table width="100%" border="1px" style="margin-top:10px;border-collapse:collapse"><thead><tr><th style="text-align:left; padding:2px">Billed By</th><th style="text-align:right; padding:2px">Billed Amount</th><th style="text-align:right; padding:2px">Payment Amount</th><th style="text-align:right; padding:2px">Balance</th></tr></thead><tbody>';
					$.each(data.records, function(i,data){ 
						total_discount_amount += parseFloat(data.discount_amount);
						total_payment_amount += parseFloat(data.payment_amount);
						total_balance += parseFloat(data.balance);
						html += "<tr>";		
						html +="<td style='text-align:left; padding:2px'>"+data.billed_for+" ("+data.billed_by_type_name+")</td>";							
						html +="<td style='text-align:right; padding:2px'>"+data.discount_amount+"</td>";
						html +="<td style='text-align:right; padding:2px'>"+data.payment_amount+"</td>";
						html +="<td style='text-align:right; padding:2px'>"+data.balance+"</td>";
						html += '</tr>';
					});
						html += "<tr>";				
						html +="<td style='text-align:right; padding:2px'><b>Total Amount</b></td>";
						html +="<td style='text-align:right; padding:2px'><b>"+total_discount_amount.toFixed(2)+"<b/></td>";
						html +="<td style='text-align:right; padding:2px'><b>"+total_payment_amount.toFixed(2)+"<b/></td>";
						html +="<td style='text-align:right; padding:2px'><b>"+total_balance.toFixed(2)+"<b/></td>";
						html += '</tr>';
					html +="</tbody></table>"
				}
				else{
				...
				WinId = window.open("", "Bill VS Payment Report","width=950,height=800,left=50,toolbar=no,menubar=YES,status=YES,resizable=YES,location=no,directories=no, scrollbars=YES"); 
				WinId.document.open();
