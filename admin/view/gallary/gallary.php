    ...
    <div class="x_content" id="iniial_collapse">
        ...
        <form method="post"  id="gallary_form" name="gallary_form" enctype="multipart/form-data" class="form-horizontal form-label-left">
            <div class="form-group">
                <label class="control-label col-md-2 col-sm-2 col-xs-12">Search By Album Name</label>
			...
			<div class="form-group">
                <label class="control-label col-md-2 col-sm-2 col-xs-12">Album Name<span class="required">*</span></label>
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text" id="album_name" name="album_name" required class="form-control col-lg-12" />
                </div>
            </div>
			<div class="form-group">
                <label class="control-label col-md-2 col-sm-2 col-xs-12">Image Title<span class="required">*</span></label>
                <div class="col-md-6 col-sm-6 col-xs-12">
	...
	load_gallary_data = function load_gallary_data(master_id){
		...
		$.ajax({
			...
			success: function(data){ 
				...
				$('#gallary_div').html('<label class="control-label col-md-2 col-sm-2 col-xs-6 img_label"></label>');	
				if(!jQuery.isEmptyObject(data.attachment)){
					$.each(data.attachment, function(i,data){				
						if($.trim(data) != ""){
							attachement_html   = '<div class="col-md-1"><img src="'+project_url+"document/gallary_attachment/"+$.trim(data.attachment)+'" class="img-thumbnail"><span class="input-group-btn"><button type="button" class="btn btn-primary btn-sm remove_img" onclick="delete_gallary_image('+data.img_id+','+master_id+')"><span class="glyphicon glyphicon-remove"></span></button></span></div>';
