...
<div class="x_panel">
    <div class="x_title">
        <h2>Female Helth Service Info</h2>
    ...
    <div class="x_content">
		...
		<div class="x_panel">
			...
			<div class="x_content adv_cl" id="iniial_collapse_adv">
				<div class="row advance_search_div alert alert-warning">	
					<div class="row">
						<label class="control-label col-md-2 col-sm-1 col-xs-12" style="text-align:right">Health Card</label>
    	...
    	<div class="dataTables_length">
        	<label>Show 
                <select size="1" style="width: 56px;padding: 6px;" id="femaleServices_Table_length" name="femaleServices_Table_length" aria-controls="femaleServices_Table">
         ...
         <div class="dataTables_filter" id="femaleServices_Table_filter">         
			<div class="input-group">
                <input class="form-control" id="search_femaleServices_field" style="" type="text">
                <span class="input-group-btn">
                    <button type="button" class="btn btn-primary has-spinner" id="search_femaleServices_button">
       ...
       <div style="height:250px; width:100%; overflow-y:scroll">
        <table id="femaleServices_Table" name="table_records" class="table table-bordered  responsive-utilities jambo_table table-striped  table-scroll ">
            <thead>
                <tr class="headings">
                    <th class="column-title" width="20%">Health Card No</th>
                    <th class="column-title" width="20%">Female Name</th>
            ...
            <tbody id="femaleServices_table_body" class="scrollable">              
                
            </tbody>
        </table>
        </div>
        <div id="femaleServices_Table_div">
            <div class="dataTables_info" id="femaleServices_Table_info">Showing <span id="from_to_limit"></span> of  <span id="total_record"></span> entries</div>
            <div class="dataTables_paginate paging_full_numbers" id="femaleServices_Table_paginate">
            </div> 
        </div>  
    </div>
</div>
...
<div class="x_panel employee_entry_cl">
    ...
    <div class="x_content" id="iniial_collapse">
        <br /> 
		<form method="post" id="female_service_form" name="female_service_form" enctype="multipart/form-data" class="form-horizontal form-label-left">	
				...
				<div class="col-md-10 col-xs-8" >
					<div class="form-group">
						...
						<div class="col-md-4 col-sm-4 col-xs-6">
							...
							<input id="female_health_card_no" name="female_health_card_no" class="form-control col-md-6 col-xs-12" type="hidden" />
						</div>
...
$(document).ready(function () {	
	...
	$('#female_service_form').iCheck({
		checkboxClass: 'icheckbox_flat-green',
		radioClass: 'iradio_flat-green'
	});	
	
	/*Auto Suggest*/	
	$("#service_center").autocomplete({
        search: function() {
        },
...
$(document).ready(function () {	
	...
	load_femaleServices_grid = function load_femaleServices_grid(search_txt){
		$("#search_femaleServices_button").toggleClass('active');		 
		var femaleServices_Table_length =parseInt($('#femaleServices_Table_length').val());
		...
		$.ajax({
			...
			async:false,
			data: {
				q: "femaleService_grid_data",
				search_txt: search_txt,
				start_date: start_date,
				ad_health_card: ad_health_card,
				end_date: end_date,
				limit:femaleServices_Table_length,
				...
				if($.trim(search_txt) == "Print"){
					...
					if(ad_health_card!=''){            
						if(!jQuery.isEmptyObject(data.records)){							
							...
							serach_areas += "Name : "+data.records[0].female_name+" <br/>";
						...
						if(!jQuery.isEmptyObject(data.records)){
							...
							$.each(data.records, function(i,data){  
								...
								html +="<td style='text-align:left; padding:2px'>"+data.female_name+"</td>";
								html +="<td style='text-align:left; padding:2px'>"+data.service_date+"</td>";
								html +="<td style='text-align:left; padding:2px'>"+data.service_center_name+"</td>";
								html +="<td style='text-align:left; padding:2px' colspan='3'>";
								var service_infos = data.service_details;
					...
					show_record_no(current_page_no, femaleServices_Table_length, data.total_records )
					
					var total_pages = data.total_pages;	
					var records_array = data.records;
					$('#femaleServices_Table tbody tr').remove();
					$("#search_femaleServices_button").toggleClass('active');
					if(!jQuery.isEmptyObject(records_array)){
						// create and set grid table row
						var colums_array=["id*identifier*hidden","health_card_no","female_name", "service_date","service_center_name"];
						...
						create_set_grid_table_row(records_array,colums_array,condition_array,"femaleServices","femaleServices_Table", 0);
						// show the showing no of records and paging for records 
						$('#femaleServices_Table_div').show();					
						// code for dynamic pagination 				
						paging(total_pages, current_page_no, "femaleServices_Table" );					
					}
					// if the table has no records / no matching records 
					else{
						grid_has_no_result("femaleServices_Table",8);
					}	
				}			
			}
		});		
	...
	load_page = function load_page(page_no){
		if(page_no != 0){
			...
			var search_txt = $("#search_femaleServices_field").val();
			load_femaleServices_grid(search_txt)
		}
	}	
	
	// function after click search button 
	$('#search_femaleServices_button').click(function(){
		var search_txt = $("#search_femaleServices_field").val();
	...
	$('#search_femaleServices_field').keypress(function(event){
		var search_txt = $("#search_femaleServices_field").val();	
		if(event.keyCode == 13){
			// every time current_page_no need to set to "1" if the user search from search bar
			current_page_no=1;
...
$(document).ready(function () {		
	...
	$('#save_button').click(function(event){		
		...
		var formData = new FormData($('#female_service_form')[0]);
		formData.append("q","insert_or_update_female_service");
	...
	clear_form = function clear_form(){			 
		$('#master_id').val('');
		$('#female_health_card_no').val('');
		$("#female_service_form").trigger('reset');		
		$('#search_by_health_card_no').val('');
		$('#female_service_form').iCheck({
			checkboxClass: 'icheckbox_flat-green',
			radioClass: 'iradio_flat-green'
		});	
		$("#female_service_form .tableflat").iCheck('uncheck');
			...
			$.ajax({
				...
				success: function(data){
					if(data.records == 'null'){
						...
						if(!jQuery.isEmptyObject(data.records)){
							$.each(data.records, function(i,data){ 						
								$('#female_health_card_no').val(data.health_card_no);
	...
	delete_femaleServices = function delete_femaleServices(femaleServices_id){
		if (confirm("Do you want to delete the record? ") == true) {
			$.ajax({
				url: url,
				type:'POST',
				async:false,
				data: "q=delete_femaleServices&femaleServices_id="+femaleServices_id,
				success: function(data){
					if($.trim(data) == 1){
	...
	edit_femaleServices = function edit_femaleServices(femaleServices_id){
		$.ajax({
			url: project_url+"controller/femaleController.php",
			dataType: "json",
			type: "post",
			async:false,
			data: {
				q: "get_femaleServices_details",
				femaleServices_id: femaleServices_id
			},
			success: function(data){
				$('#master_id').val(femaleServices_id);
				...
				if(!jQuery.isEmptyObject(data.female_info)){
						var master_info = data.female_info;
						$('#search_by_health_card_no').val(master_info.health_card_no);
						$('#female_health_card_no').val(master_info.health_card_no);
	...
	$('#adv_search_button').click(function(){
		load_femaleServices_grid("Advance_search");
	});
	
	//print advance search data
	$('#adv_search_print').click(function(){
		load_femaleServices_grid("Print");
	});
