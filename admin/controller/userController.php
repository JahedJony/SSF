...
switch ($q){
		...
		if(isset($emp_id) && $emp_id == ""){
			...
			if(is_numeric($return) && $return>0) {
				...
				if($health_card_no != ""){
					...
					$return_health = $dbClass->update("helth_card",$columns_value, $condition_array);	
				} 
	...
	case "grid_data":	
		...
		if($search_txt == "Print" || $search_txt == "Advance_search"){		
			...
			if($health_card_status == 2){}
				...
				$condition  .=" AND e.health_card_no is not null ";	
			}
			else{
				$condition  .=" AND e.health_card_no is null ";
			}	
		}
		// textfield search for grid
		else{
			$condition .=	" where a.is_active =1 and CONCAT(e.emp_id, e.full_name, e.designation_name) LIKE '%$search_txt%' ";			
		}
		...
		if($employee_grid_permission==1 || $permission_grid_permission==1){
			$sql = "SELECT e.emp_id, e.full_name, e.designation_name, e.contact_no, e.photo, ifnull(e.health_card_no,'') health_card_no, a.is_active, a.user_name,
					...
					join appuser a on a.user_id=e.emp_id				
					$condition  
					order by emp_id asc limit $start, $end";	
	...
	case "get_emp_details":
		...
		if($update_permission==1){
			$emp_details = $dbClass->getResultList("select inf.emp_id, inf.full_name, inf.designation_name, ifnull(inf.health_card_no,'') health_card_no,
													inf.photo, inf.contact_no, app.user_name,
													inf.email, inf.blood_group, inf.is_active_home_page, inf.remarks, app.is_active, photo
													from user_infos inf 
													join appuser app on inf.emp_id=app.user_id where emp_id='$emp_id'");
	...
	case "get_user_info":
		$emp_info = "select inf.emp_id, inf.full_name, inf.designation_name, inf.photo, app.user_name, ifnull(inf.health_card_no,'') health_card_no,
