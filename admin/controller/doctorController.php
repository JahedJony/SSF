...
switch ($q){
	...
	case "insert_or_update":	
		if(isset($master_id) && $master_id == ""){
			if(isset($_FILES['doctor_image_upload']) && $_FILES['doctor_image_upload']['name']!= ""){
				$desired_dir = "../images/doctor";
				chmod( "../images/doctor", 0777);
				$file_name = $_FILES['doctor_image_upload']['name'];
				$file_size =$_FILES['doctor_image_upload']['size'];
				$file_tmp =$_FILES['doctor_image_upload']['tmp_name'];
				$file_type=$_FILES['doctor_image_upload']['type'];	
			...
			$return = $dbClass->insert("doctor", $columns_value);
			...
			$prev_attachment = $dbClass->getSingleRow("select image from doctor where id=$master_id");
			if($photo != ""){	
				...
				$return = $dbClass->update("doctor",$columns_value, $condition_array);				
			}
			$is_active=0;
			if(isset($_POST['is_active'])){
				$is_active=1;
			...
			$return_details = $dbClass->update("doctor",$columns_value, $condition_array); 
	...
	case "grid_data":	
		...
		if($search_txt == "Print" || $search_txt == "Advance_search"){
			$condition .=" 	speciality like '%$doc_speciality%' AND address LIKE '%$doc_address%'";		
		...
		if($employee_grid_permission==1 || $permission_grid_permission==1){
			$sql = "select id,name,speciality,address,mobile_no,image,remarks,username,`status`, ifnull(email,'') email,
	...
	case "get_doctor_details":
		...
		$emp_details = $dbClass->getResultList("select id, name, speciality, address, mobile_no, username, email,
											   image, remarks, username, `status` 
											   from doctor where id = '$doctor_id'");
	...
	case "get_doctor_details_personal":
		//$update_permission = $dbClass->getUserGroupPermission(12);
		//if($update_permission==1){
		$emp_details = $dbClass->getResultList("select id, name, speciality, address, mobile_no, username, email,
											   image, remarks, username, `status` 
	...
	case "update_information":
		$password = $dbClass->getSingleRow("select password from doctor where id=$loggedUser");
		if(isset($new_password) && $new_password != ""){
			...
			if($password['password'] == md5($old_password)){
				...
				$return_pass = $dbClass->update("doctor", $columns_value, $condition_array);
